 ...
 * datestr,fill,all_days
 * @type {Object}
 * {"datestr":datestr,"fill":'',"all_days":''}
 */
var getDayList=function(obj){
	/**
	...
	function getLastDaysInMonth(year,month){ 
	      ...
	      month=='12'?(month=1,year+=1):month+=1
	      m4=month
	      var ldays3=(new Date(year,month,0)).getDate();
	      var dayarr=[ldays0,ldays1,ldays2,ldays3];//包含 上月，本月，下月 的最后一天的日期是几号
	      if(config_param.isLog)console.log(dayarr);
	      return dayarr;
	}
	var date,m2,m3,toDay,need_days;
	obj.datestr?date=new Date(obj.datestr):date=new Date();
...
//	var date=new Date();
	...
	clist[m2]=crr_m	
	need_days = crr_m.length + days[2]+days[3]
	console.log(need_days)
//	var all_days = 42-wday//页面填充的日期总个数
	var all_days =obj.all_days || need_days//页面填充的日期总个数
